 ...
 * @author prodigy
 */
public class updateProjectValidatorController implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        //System.out.println("====Into update project validator===");
        try {
            Project project = (Project) target;
            String projname = project.getProj_name();
            String startdate = project.getStart_date();
            String enddate = project.getEnd_date();
            //System.out.println("enddate in validator: "+enddate);
            ...
            try {
                if (!enddate.equals("")) {
                    ...
                    String delenddate = enddate.replaceAll("/", "");
                    Calendar ed = new GregorianCalendar(Integer.parseInt(delenddate.substring(4, 8)), Integer.parseInt(delenddate.substring(2, 4)), Integer.parseInt(delenddate.substring(0, 2)));
                    if (ed.before(st)) {
                        errors.rejectValue("end_date", "Invalid.end_date", "End Date can't be less than Start date");
                    }
                }

            } catch (Exception e) {
                //System.out.println("error in validator enddate: "+e);
            }
            if (tg.before(st)) {
            ...
            //System.out.println("error in project validator: "+e);
        }
    }
